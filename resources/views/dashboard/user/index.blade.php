<x-app-layout :title=$pageTitle :sub-title=$subTitle :icon=$icon :crumbs=$crumbs>
    ...
    {{-- create user --}}
    <x-ui.modal id="addModal" title="Tambah User" :action="route('user.store')" :fallback="true">
        ...
        <x-ui.input label="Nama Legkap" id="name" name="name" required placeholder="Nama"
            value="{{ old('name') }}" />
        ...
        <x-ui.input label="Password" id="password" name="password" required placeholder="password"
            value="{{ old('password') }}" />
